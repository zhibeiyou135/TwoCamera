...
// 全局初始化锁，确保PyTorch只初始化一次
std::mutex pytorch_init_mutex;
bool pytorch_initialized = false;

TorchEngine_DV::TorchEngine_DV(const QString &model) {
#ifdef TORCH_ENABLE
  qDebug() << "DV模型路径:" << model;
  ...
  if (!modelFile.exists()) {
    qDebug() << "错误: DV模型文件不存在:" << model;
    throw std::runtime_error("DV模型文件不存在");
  }

  if (modelFile.size() == 0) {
    qDebug() << "错误: DV模型文件为空:" << model;
    throw std::runtime_error("DV模型文件为空");
  }

  qDebug() << "DV模型文件大小:" << modelFile.size() << "字节";
  ...
  try {
    ...
    try {
      ...
      qDebug() << "DV模型预热失败（非致命错误）:" << e.what();
    } catch (...) {
      qDebug() << "DV模型预热失败（未知异常，非致命错误）";
    }
    
  } catch (const std::exception& e) {
    qDebug() << "DV模型加载失败:" << e.what();
    throw;
  } catch (...) {
    qDebug() << "DV模型加载失败: 未知异常";
    throw;
  }
#else
  qDebug() << "TORCH_ENABLE未定义，跳过DV模型加载";
  throw std::runtime_error("PyTorch支持未启用");
#endif
}

cv::Mat *TorchEngine_DV::processInput(cv::Mat *input) {
  // 检查输入图像是否有效
  if (!input || input->empty() || input->data == nullptr) {
    qDebug() << "TorchEngine_DV: 输入图像为空，返回空指针";
    return nullptr;
  }
  ...
  cv::Mat *resize_out = new cv::Mat();
    ...
    qDebug() << "TorchEngine_DV processInput 异常:" << e.what();
    return nullptr;
  } catch (...) {
    qDebug() << "TorchEngine_DV processInput 未知异常";
    return nullptr;
  }
}
...
void TorchEngine_DV::NMS(std::vector<Box> &boxes) {
  ...
  boxes = result;
  qDebug() << "DV Enhanced NMS result: " << boxes.size() << "boxes";
}
...
cv::Mat *TorchEngine_DV::processOutput(cv::Mat *img, cv::Mat *output,QString & Labels) {
  ...
  for (int i = 0; i < output->rows; i++) {
    ...
    if (output->at<float>(i, 4) > detectionConfig.confidence_threshold && output->at<float>(i, 4) < 1) {
      ...
      Labels="DV识别结果:"+QString::fromStdString(name[max_id])+" ";
  ...
  qDebug() << "DV box num before NMS: " << boxes.size();

  if(boxes.size()<1)
    Labels+="无";
  else {
    NMS(boxes);
    qDebug() << "DV最终检测数量:" << boxes.size();
  }

  paintbox(*img, boxes);
  return img;
}
void TorchEngine_DV::paintbox(cv::Mat &img, const std::vector<Box> &boxes) {
  for (Box box : boxes) {
    qDebug() << "检测类别:" << box.cls;
    ...
    cv::rectangle(img,
                  cv::Point(box.left % DV_HUGE_STEP * DV_WIDTH_SCALE,
                            box.top % DV_HUGE_STEP * DV_HEIGHT_SCALE-50),
                  cv::Point(box.right % DV_HUGE_STEP * DV_WIDTH_SCALE,
                            box.bottom % DV_HUGE_STEP * DV_HEIGHT_SCALE-50),
                  color, 10);
    // cv::putText(img, name[box.cls],
    //             cv::Point(box.left % DV_HUGE_STEP * DV_WIDTH_SCALE,
    //                       box.top % DV_HUGE_STEP * DV_HEIGHT_SCALE-70),
    //             cv::FONT_HERSHEY_COMPLEX, 5, cv::Scalar(255, 0, 0), 3);
  }
}
cv::Mat *TorchEngine_DV::infer(cv::Mat *tensor, QString & Labels) {
  if (!tensor || tensor->empty()) {
    Labels = "DV识别结果:输入图像无效";
    return nullptr;
  }
  ...
  try {
    ...
    try {
      ...
      if (out.numel() * sizeof(float) <= outmap.total() * outmap.elemSize()) {
        ...
        Labels = "DV识别结果:输出大小错误";
      }
      
    } catch (const std::exception& e) {
      qDebug() << "DV推理过程异常:" << e.what();
      Labels = "DV识别结果:推理异常";
    } catch (...) {
      qDebug() << "DV推理过程未知异常";
      Labels = "DV识别结果:推理异常";
    ...
    qDebug() << "TorchEngine_DV infer 异常:" << e.what();
    Labels = "DV识别结果:推理异常";
    ...
    qDebug() << "TorchEngine_DV infer 未知异常";
    Labels = "DV识别结果:未知异常";
...
// 加载检测配置
void TorchEngine_DV::loadDetectionConfig() {
  try {
    QString configPath = ConfigManager::getInstance().getConfigPath();
    QFile configFile(configPath);

    if (!configFile.open(QIODevice::ReadOnly)) {
      qDebug() << "无法打开配置文件，使用默认DV检测配置:" << configPath;
      return;
    }
    ...
    // 加载DV配置
    if (parametersObj.contains("dv") && parametersObj["dv"].isObject()) {
      QJsonObject dvConfig = parametersObj["dv"].toObject();
      detectionConfig.loadFromJson(dvConfig, "TorchEngine_DV");
    } else {
      qDebug() << "未找到dv配置，使用默认值";
    }

  } catch (const std::exception& e) {
    qDebug() << "加载DV检测配置时发生异常:" << e.what();
  }
}
...
bool TorchEngine_DV::isValidDetection(const Box &box) {
  ...
  if (detectionConfig.enable_border_filter) {
    ...
    const float img_width = WIDTH * DV_WIDTH_SCALE;
    const float img_height = HEIGHT * DV_HEIGHT_SCALE;
